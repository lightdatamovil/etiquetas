    ...
    try {
        ...
        if (result.length === 0) {
            return res.status(404).json({
                error: "No se encontraron datos para la empresa o envio proporcionados.",
            })
        }
        const checkEnvio = "SELECT did From envios where did = ? and didCliente = ? and superado = 0 and elim = 0"
        const result2 = await executeQuery(connection, checkEnvio, [didEnvio, didCliente])
        if (result2.length === 0) {
