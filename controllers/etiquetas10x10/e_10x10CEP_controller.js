        ...
        if (camposEspeciales.length > 0) {
            ...
            await camposEspeciales.map((campo) => {
                if (siguiente < 5) {
                    ...
                    doc.fontSize(tamañoCE)
                        .font("Helvetica")
                        .text(esDatoValido(campo["valor"]) ? cortarTexto(campo["valor"], 60) : "Sin información", posicionAnchoTexto3 + anchoTextoEsp + 15, posicionAltoTexto3(siguiente), { baseline: "middle", lineBreak: false })
                    siguiente += 1
                }
            })
        }
