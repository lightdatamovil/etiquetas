...
const e10x10AP = async ({ doc, objData, llevaCodigo, sinEan, camposExtraGrande, loteEnItems }) => {
    ...
    for (let i = 0; i < bultos; i++) {
        ...
        SVGtoPDF(doc, iconCalendarChico, posicionAnchoTexto1, posicionAltoTexto1(0) + 9)
        ...
        borderRadius3 = 2
        distanciaAncho3 = 5
        distanciaAlto3 = distanciaAlto1 + 173
            ...
            await camposEspeciales.map((campo) => {
                if (siguiente < 6) {
                    ...
                    if (siguiente == 0 || siguiente % 2 == 0) {
                        doc.roundedRect(distanciaAncho3, containerSiguiente3(distanciaCE), 134, altoContainer3, borderRadius3).fillAndStroke(colorGrisClaro, colorGrisClaro)
                        doc.fillAndStroke("black", "black")
                        doc.fontSize(tamañoCE)
                            .font("Helvetica-Bold")
                        ...
                        doc.roundedRect(134 + 11, containerSiguiente3(distanciaCE), 134, altoContainer3, borderRadius3).fillAndStroke(colorGrisClaro, colorGrisClaro)
                        doc.fillAndStroke("black", "black")
                        doc.fontSize(tamañoCE)
                            .font("Helvetica-Bold")
                            .text(esDatoValido(campo["nombre"]) ? cortarTexto(campo["nombre"], 15) + ":" : "CampoEsp:", 134 + 11 + padding3, posicionAltoTexto3(distanciaCE), { baseline: "middle", lineBreak: false })
        ...
        if (fulfillment.length > 0) {
            ...
            await fulfillment.map((elemento) => {
                ...
                if (sinEan) {
                    ...
                    if (loteEnItems) {
                        doc.roundedRect(distanciaAncho4 + 104 + margin4 * 2, distanciaAlto4, 96, altoContainer4, borderRadius4).fillAndStroke(colorGrisClaro, colorGrisClaro)
                        doc.roundedRect(distanciaAncho4 + 200 + margin4 * 3, distanciaAlto4, 38, altoContainer4, borderRadius4).fillAndStroke(colorGrisClaro, colorGrisClaro)
                    } else {
