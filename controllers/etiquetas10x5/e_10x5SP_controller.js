...
const { iconCalendarChico, iconTelefono, iconUbicacion, iconNombre } = require("../../utils/icons.js")
...
//ETIQUETA 10x5 CON SOLO PREMIUM

const e10x5SP = async ({ doc, objData, llevaCodigo }) => {
    ...
    for (let i = 0; i < bultos; i++) {
        ...
        tamañoFuente2 = tamañoFuente1
        anchoContainer2 = 141
        altoContainer2 = altoContainer1
        margin2 = 6
        padding2 = 7
        borderRadius2 = 2
        distanciaAncho2 = 5
        ...
        doc.roundedRect(distanciaAncho2 + anchoContainer2 + margin2, containerSiguiente2(0), anchoContainer2 - 15, altoContainer2, borderRadius2).fillAndStroke(colorGrisClaro, colorGrisClaro)
        doc.roundedRect(distanciaAncho2, containerSiguiente2(1), anchoContainer2 + anchoContainer2 - 10, altoContainer2, borderRadius2).fillAndStroke(colorGrisClaro, colorGrisClaro)
        ...
        doc.fontSize(tamañoNombre)
            .font("Helvetica-Bold")
            .text(esDatoValido(nombre) ? cortarTexto(nombre, 32) : "Sin información", posicionAnchoTexto2 + 15, posicionAltoTexto2(0), { baseline: "middle", lineBreak: false })

        SVGtoPDF(doc, iconTelefono, posicionAnchoTexto2 + anchoContainer2 + padding2, posicionAltoTexto2(0) - 5)
        let tamañoNumero = tamañoSegunLargo(nroTelefono, tamañoFuente2, 24)
        doc.fontSize(tamañoNumero)
            .font("Helvetica-Bold")
            .text(esDatoValido(nroTelefono) ? cortarTexto(nroTelefono, 32) : "Sin información", posicionAnchoTexto2 + anchoContainer2 + padding2 + 15, posicionAltoTexto2(0), { baseline: "middle", lineBreak: false })
        ...
        doc.fontSize(tamañoDireccion)
            .font("Helvetica-Bold")
            .text(`${esDatoValido(direccion) ? cortarTexto(direccion, 70) : "Sin información"} ${esDatoValido(cp) ? "CP: " + cortarTexto(cp, 10) : ""}`, posicionAnchoTexto2 + 15, posicionAltoTexto2(1), { baseline: "middle", lineBreak: false })
