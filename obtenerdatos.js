...
const obtenerDatosEnvios = async (idempresa, dids) => {
  try {
    ...
    let logo =
      cacheLogos[empresa.empresa] ||
      `${empresa.url}/app-assets/images/logo/logo.png` ||
      null;
    ...
    const datos = Object.fromEntries(
      consultas.map((c, i) => [c.key, resultados[i]])
    );
    ...
    datos.envios.forEach((envio) => {
      enviosMap[envio.did] = {
        ...
        fecha_venta:
          envio.fecha_venta !== "0000-00-00 00:00:00"
            ? envio.fecha_venta
            : envio.fecha_inicio !== "0000-00-00 00:00:00"
            ? envio.fecha_inicio
            : null,
        ...
        qr:
          envio.flex == 1
            ? envio.ml_qr_seguridad
            : `{"local": 1, "did": "${envio.did}", "cliente": ${envio.didCliente}, "empresa": ${idempresa}}`,
...
const registrarReimpresion = async (idempresa, dids, modulo, quien) => {
  ...
  if (!Array.isArray(dids) || dids.length === 0) {
    throw new Error(
      "El parámetro dids debe ser un array con al menos un elemento."
    );
  }
  ...
  try {
    ...
    let queryUpdate = `UPDATE envios SET impreso = 1 WHERE did IN (${dids
      .map(() => "?")
      .join(", ")}) AND superado = 0 AND elim = 0`;
